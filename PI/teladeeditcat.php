...
if (isset($_GET['id_cat'])) {
    ...
    if ($categoria) {
        if (isset($_POST['salvar'])) {
            ...
            $update = $conn->prepare("UPDATE categoria SET nome_cat = :nome_cat WHERE id_cat = :id_cat");
            $update->bindValue(":nome_cat", $novo_nome);
            ...
            echo '<a href="teladecat.php" class="btn btn-secondary">Voltar para a Tabela</a>';
            exit();
        }
        ?>
        <div class="centro">
            <div class="caixa">
                <div class="row">
                    ...
                    <form action="teladeeditcat.php?id_cat=<?php echo $id_cat; ?>" method="POST">
                        <input type="hidden" name="id_cat" value="<?php echo $id_cat; ?>" />
                        
                        <div class="mb-3">
                            <label for="nome_categoria" class="form-label">Categoria selecionada:</label>
                            <input type="text" name="nome_categoria" id="nome_categoria" class="form-control" 
                                   value="<?php echo htmlspecialchars($categoria['nome_cat']); ?>" required />
                        </div>
                        <input type="submit" name="salvar" value="Salvar" class="btn btn-primary" />
                        <a href="teladecat.php" class="btn btn-secondary">Voltar</a>
                    </form>
                </div>
            </div>
        </div>
